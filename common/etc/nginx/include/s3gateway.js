...
/**
 * Resolves the S3 bucket name for the current request.
 *
 * Uses the per-request nginx variable $s3_bucket_name_var when set (populated
 * by the X-S3-Bucket header whitelist map in default.conf.template), falling
 * back to the process-level S3_BUCKET_NAME environment variable for requests
 * that carry no header or an unlisted bucket name.
 *
 * @param r {NginxHTTPRequest} HTTP request object
 * @returns {string} resolved S3 bucket name
 * @private
 */
function _resolveBucket(r) {
    return (r.variables.s3_bucket_name_var && r.variables.s3_bucket_name_var !== '')
        ? r.variables.s3_bucket_name_var
 ...
 *
 * The bucket name is resolved per-request via _resolveBucket(): if the
 * X-S3-Bucket header was set to a whitelisted value the nginx variable
 * $s3_bucket_name_var carries it; otherwise the env default is used.
 *
 * @param r {NginxHTTPRequest} HTTP request object
 * @returns {string} AWS authentication signature
 */
function s3auth(r) {
    const bucket = _resolveBucket(r);
 ...
 * the bucket name.
 *
 * The bucket name is resolved per-request via _resolveBucket() so that
 * requests carrying X-S3-Bucket use the correct path prefix.
 *
 * @param r {NginxHTTPRequest} HTTP request object
